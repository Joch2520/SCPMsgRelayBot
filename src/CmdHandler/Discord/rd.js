...
module.exports.run = (clients, message, args) => {
  ...
  if (isNaN(num)||isNaN(side)) {
    let ReturnEmbed = {
      color: 0x660000,
      title: "用法：",
      description: clients.config.CMD_PREFIX+"rd XdY±Z (F)\nX為骰子數目，Y為骰子面數，Z為調整數，F為調整選項。",
      fields: [
        {
          name: `調整選項: "-e" / "--each"`,
    ...
    return message.channel.send({embed:ReturnEmbed})
  }
